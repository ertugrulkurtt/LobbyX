...
export const getUserStats = async (userId: string): Promise<UserStats> => {
  try {
    ...
    if (error?.code === "permission-denied") {
      ...
      throw new Error(
        `İstatistikler yüklenemedi: ${error.message || "Bilinmeyen hata"}`,
      );
    }
  }
};
...
export const updateUserStats = async (
  ...
  try {
    ...
    if (error?.code === "permission-denied") {
      console.warn(
        "🔒 User stats update disabled - Firestore rules need deployment",
      );
...
export const trackVoiceTime = async (userId: string, minutes: number) => {
  try {
    ...
    // Update daily activity
    const today = new Date().toISOString().split("T")[0];
    await updateDailyActivity(userId, today, {
      voiceTime: minutes,
      activeTime: minutes,
    });
  } catch (error: any) {
...
export const calculateUserRank = async (userId: string): Promise<number> => {
  try {
    ...
    if (error?.code === "permission-denied") {
      console.warn(
        "🔒 Rank calculation disabled - Firestore rules need deployment",
      );
...
export const subscribeToUserStats = (
  ...
  return onSnapshot(
    ...
    (error) => {
      ...
      if (error.code === "permission-denied") {
        console.warn(
          "🔒 User stats subscription disabled - Firestore rules need deployment",
        );
...
export const trackDailyLogin = async (userId: string) => {
  try {
    ...
    if (error?.code === "permission-denied") {
      console.warn(
        "🔒 Daily login tracking disabled - Firestore rules need deployment",
      );
    } else if (error?.code === "unauthenticated") {
