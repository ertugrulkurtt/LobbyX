...
export const sendFriendRequest = async (
  ...
  return withRetry(async () => {
    ...
    try {
      ...
      const fromUserName =
        fromUserData?.displayName || fromUserData?.username || "Unknown User";
...
export const acceptFriendRequest = async (requestId: string): Promise<void> => {
  try {
    ...
    try {
      ...
      const toUserName =
        toUserData?.displayName || toUserData?.username || "Unknown User";
 ...
 */
export const markAllFriendNotificationsAsRead = async (
  userId: string,
): Promise<void> => {
  try {
    const notificationsRef = collection(db, "notifications");
    ...
    const updatePromises = notificationSnapshot.docs
      .filter((doc) => {
        const data = doc.data();
        return (
          data.type === "friend_request" || data.type === "friend_accepted"
        );
      })
      .map((notificationDoc) =>
        updateDoc(notificationDoc.ref, {
...
export const getUserConversations = async (
  ...
  try {
    ...
    if (error?.code === "permission-denied") {
      console.warn(
        "🔒 Conversations disabled - Firestore rules need deployment",
      );
...
export const subscribeToUserFriends = (
  userId: string,
  callback: (friends: RealUser[]) => void,
) => {
...
export const subscribeToFriendRequests = (
  ...
  const unsubscribeIncoming = onSnapshot(
    ...
    (error) => {
      ...
      if (error.code === "permission-denied") {
        console.error(
          "Permission denied: Please check Firestore rules for friendRequests collection",
        );
      }
    },
  );
