...
export function FirebaseStatus() {
  ...
  const [status, setStatus] = useState<
    "checking" | "connected" | "permissions_needed"
  >("checking");
    ...
    <div className="bg-neon-orange/10 border border-neon-orange/30 rounded-lg p-3">
      ...
      {showDetails && (
        <div className="mt-2 text-xs text-gaming-muted">
          <p className="mb-2">
            Firebase bağlantı sorunu tespit edildi. Bu durum şu sebeplerden
            olabilir:
          </p>
          <div className="bg-gaming-surface rounded p-2 text-xs space-y-1">
            <div>
              🔐 <strong>Firestore Rules:</strong> Kurallar deploy edilmemiş
              olabilir
            </div>
            <div>
            ...
            <div>
              👤 <strong>Authentication:</strong> Oturum açma sorunu
            </div>
          </div>
