...
import { Phone, ArrowLeft, AlertCircle, CheckCircle, RefreshCw } from 'lucide-react';
...
export default function PhoneVerification({ phoneNumber, onVerified, onBack }: PhoneVerificationProps) {
  ...
  // Mock function to send SMS verification code
  const sendSMSCode = async () => {
    setIsResending(true);
    try {
      // Simulate API call to send SMS verification code
      await new Promise(resolve => setTimeout(resolve, 1500));
      setNotification({ type: 'success', message: 'Doğrulama kodu SMS ile gönderildi!' });
      setResendCooldown(60); // 60 seconds cooldown
    } catch (error) {
      setNotification({ type: 'error', message: 'SMS gönderilirken hata oluştu' });
    } finally {
      setIsResending(false);
    }
  };

  // Mock function to verify SMS code
  const verifySMSCode = async (inputCode: string): Promise<boolean> => {
    // Simulate API verification
    await new Promise(resolve => setTimeout(resolve, 1000));
    // For demo purposes, accept "654321" as valid SMS code
    return inputCode === '654321';
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isValid) {
        setNotification({ type: 'success', message: 'Telefon numarası doğrulandı!' });
        setTimeout(() => {
          onVerified();
        }, 1500);
      } else {
  ...
  // Send initial SMS code on mount
  useEffect(() => {
    sendSMSCode();
  }, []);
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gaming-muted text-sm">
          <span className="text-neon-green font-medium">{formatPhoneNumber(phoneNumber)}</span> numarasına gönderilen 6 haneli kodu girin
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-gaming-text">SMS Doğrulama Kodu</label>
          <input
            type="text"
            value={code}
            ...
            className={`w-full px-4 py-4 text-center text-2xl font-mono tracking-widest bg-gaming-surface/50 backdrop-blur-sm border rounded-xl focus:outline-none focus:ring-2 transition-all duration-300 ${
              errors 
                ...
                : 'border-gaming-border focus:ring-neon-green/50 focus:border-neon-green'
            }`}
            placeholder="000000"
            maxLength={6}
        ...
        <div className="text-center">
          <button
            ...
            className={`text-neon-green hover:text-neon-cyan transition-colors text-sm font-medium inline-flex items-center space-x-2 ${
          ...
          >
            ...
            <span>
              {resendCooldown > 0 
                ? `SMS tekrar gönder (${resendCooldown}s)`
                : isResending
                ? 'Gönderiliyor...'
                : 'SMS tekrar gönder'
              }
