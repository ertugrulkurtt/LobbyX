...
export default function FirebaseRulesStatus({ onClose }: FirebaseRulesStatusProps) {
  ...
  const getStatusIcon = () => {
    if (isLoading) return <RefreshCw className="w-5 h-5 animate-spin text-blue-400" />;
    if (isConnected === true) return <CheckCircle className="w-5 h-5 text-green-400" />;
    if (isConnected === false) return <AlertTriangle className="w-5 h-5 text-red-400" />;
          ...
          {isConnected === false && (
            <div className="space-y-2">
              <div className="bg-red-500/10 border border-red-500/20 rounded p-2 text-xs text-red-400">
                <div className="font-medium mb-1">Kuralları deploy etmek için:</div>
              ...
              <div className="text-xs text-gaming-muted">
                <div>1. Firebase CLI: <code>npm i -g firebase-tools</code></div>
                <div>2. Giriş: <code>firebase login</code></div>
                <div>3. Deploy: <code>firebase deploy --only firestore:rules</code></div>
              </div>
            </div>
          )}
