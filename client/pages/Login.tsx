...
type LoginStep = 'form' | 'device-verification';
...
export default function Login() {
  ...
  // Mock function to check if device is known
  const checkDeviceKnown = async (): Promise<boolean> => {
    // Simulate API call to check device fingerprint
    await new Promise(resolve => setTimeout(resolve, 1000));
    // For demo purposes, always return false (unknown device)
    return false;
  };

  // Mock function to get user contact info
  const getUserContactInfo = async (emailOrPhone: string) => {
    // Simulate API call to get user's email and phone
    await new Promise(resolve => setTimeout(resolve, 500));
    // For demo purposes, return mock data
    if (emailOrPhone.includes('@')) {
      setUserEmail(emailOrPhone);
      setUserPhone('+90 555 123 45 67'); // Mock phone
    } else {
      setUserPhone(emailOrPhone);
      setUserEmail('user@example.com'); // Mock email
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isDeviceKnown = await checkDeviceKnown();
      
      if (!isDeviceKnown) {
        // Unknown device, require additional verification
        await getUserContactInfo(formData.emailOrPhone);
        setNotification({ 
          type: 'success', 
          message: 'Kimlik doğrulandı! Bu cihazdan daha önce giriş yapılmadığı için ek doğrulama gerekiyor...' 
        });
        setTimeout(() => {
          setCurrentStep('device-verification');
        }, 2000);
      } else {
  ...
  const handleDeviceVerified = () => {
    setNotification({ type: 'success', message: 'Cihaz doğrulandı! Dashboard\'a yönlendiriliyorsunuz...' });
    setTimeout(() => {
      navigate('/dashboard');
    }, 1500);
  };
