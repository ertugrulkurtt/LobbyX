...
type RegistrationStep = 'form' | 'email-verification' | 'phone-verification' | 'completed';
...
export default function Register() {
  ...
  const handleInputChange = (field: keyof RegisterData | 'confirmPassword', value: string) => {
    ...
    // Real-time validation (privacyAgreement is handled separately)
    validateField(field, value);
  };

  const handlePrivacyAgreementChange = (checked: boolean) => {
    setPrivacyAgreement(checked);
    // Only clear the error if the user checks the box, don't validate immediately
    if (checked && errors.privacyAgreement) {
      const newErrors = { ...errors };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Manual privacy agreement validation only on submit
    const newErrors = { ...errors };
    if (!privacyAgreement) {
    ...
    try {
      // Form validation passed, proceed to email verification
      setNotification({ type: 'success', message: 'Form doğrulandı! E-posta doğrulamasına geçiliyor...' });
      setTimeout(() => {
        setCurrentStep('email-verification');
      }, 1500);
    } catch (error: any) {
      setNotification({ type: 'error', message: error.message || 'Form doğrulama başarısız' });
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gaming-bg flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-4xl relative z-10">
        ...
        <div className="card-glass animate-scale-in backdrop-blur-xl bg-gaming-surface/20 border border-gaming-border/50">
          ...
          <div className="mb-6">
            {currentStep === 'form' && (
              <h2 className="text-2xl font-bold text-gaming-text text-center">Kayıt Ol</h2>
            )}
            {currentStep === 'email-verification' && (
              <div className="text-center">
                <h2 className="text-2xl font-bold text-gaming-text mb-2">E-posta Doğrulama</h2>
                <div className="flex items-center justify-center space-x-2 text-sm text-gaming-muted">
                  <div className="w-2 h-2 bg-neon-green rounded-full"></div>
                  <span>Form Tamamlandı</span>
                  <div className="w-2 h-2 bg-neon-cyan rounded-full animate-pulse"></div>
                  <span>E-posta Doğrulama</span>
                  <div className="w-2 h-2 bg-gaming-border rounded-full"></div>
            ...
            {currentStep === 'phone-verification' && (
              <div className="text-center">
                <h2 className="text-2xl font-bold text-gaming-text mb-2">Telefon Doğrulama</h2>
                <div className="flex items-center justify-center space-x-2 text-sm text-gaming-muted">
                  <div className="w-2 h-2 bg-neon-green rounded-full"></div>
                  <span>Form Tamamlandı</span>
                  <div className="w-2 h-2 bg-neon-green rounded-full"></div>
            ...
            {currentStep === 'completed' && (
              <div className="text-center">
                <h2 className="text-2xl font-bold text-neon-green mb-2">Kayıt Tamamlandı!</h2>
                <div className="flex items-center justify-center space-x-2 text-sm text-gaming-muted">
                  <div className="w-2 h-2 bg-neon-green rounded-full"></div>
                  <span>Form Tamamlandı</span>
                  <div className="w-2 h-2 bg-neon-green rounded-full"></div>
          ...
          {currentStep === 'form' && (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-gaming-text">��ifre</label>
                    <div className="relative">
          ...
          {currentStep === 'completed' && (
            <div className="text-center space-y-6">
              <div className="w-20 h-20 bg-gradient-to-br from-neon-green to-neon-cyan rounded-2xl mx-auto mb-4 flex items-center justify-center animate-pulse-glow">
                <CheckCircle className="w-10 h-10 text-white" />
              </div>
              <div>
                <h3 className="text-xl font-bold text-neon-green mb-2">Kayıt Başarılı!</h3>
                <p className="text-gaming-muted">
                  Hoş geldin! Dashboard'a yönlendiriliyorsun...
                </p>
              </div>
              {notification && (
                <div className="p-4 rounded-xl border bg-neon-green/10 border-neon-green/30 text-neon-green">
                  <div className="flex items-center justify-center space-x-2">
                    <CheckCircle className="w-5 h-5" />
                    <span className="text-sm font-medium">{notification.message}</span>
                  </div>
